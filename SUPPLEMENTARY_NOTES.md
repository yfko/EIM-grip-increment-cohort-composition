...
**Fixed design.** All 134 assessed adults; dominant-hand maximum of six grip trials as the primary outcome, non-dominant hand as sensitivity; demographic baseline sex, age, BMI; validity filter R > 0, X > 0, phase angle > 0; per-channel two-tailed 4 SD rule on ln(phase angle); one (R, X) pair per channel, phase angle not entered; primary EIM feature set the four forearm channels at 100 kHz (8 features); subject-level 10 × 5-fold cross-validation stratified on sex and the grip median with inner 5-fold tuning; Frisch–Waugh–Lovell residualization of outcome and features on demographics fitted within training folds; median imputation within folds; primary metric ΔMAE against the demographic baseline with 2,000-resample subject bootstrap; secondary ΔR² and AUC; seed 20260904. The bootstrap draws from a single module-level random number stream, so resampling depends on the order in which models are evaluated within a run; the figures reported are those of the deposited run, and a re-run that evaluates the models in a different order reproduces the primary increment to two decimal places but may differ by 0.01 kg in the secondary gradient-boosting contrasts.
...
**Model set.** Lasso on raw features plus demographics (literature protocol); elastic net on residualized features (H1); sparse-group Lasso over ten site-by-frequency groups (results to be reported separately); PLS; XGBoost with and without monotone constraints; RBF kernel ridge; bilateral mixed-effects model; L2 logistic regression for AWGS weakness.
...
**Observed outcomes of the decision rules.** (a) XGBoost minus elastic net 0.13 kg (−0.14 to 0.39): no non-linear increment. (b) Sparse-group model results are outside the scope of this paper and will be reported separately. (c) Bilateral model interval wider than the elastic net's (0.14, −0.27 to 0.44): not adopted.
...
## Supplementary Note 4. Comparison with the published forearm EIM protocol

The published protocol used 396 features from 33 frequencies, two compartments and two electrode configurations, fitted by Lasso to a single 70:30 split of 65 to 69 participants per analysis (Mahato et al., 2025). The present study used eight features, the resistance and reactance of four forearm channels at 100 kHz, fitted under ten-times-repeated five-fold subject-level cross-validation in 134 participants, with the impedance features adjusted for sex, age and BMI within training folds (Methods 2.4 of the main text). With this smaller feature set the published end-point for EIM with demographics, R² of about 0.64, was reproduced (Table 2 of the main text); the difference between the two studies lies in the order of decomposition, discussed in Section 4.2 of the main text.

*Reference cited in this note* (numbered [3] in the main text's reference list): N. K. Mahato, B. S. Cobb, S. B. Rutkove, W. D. Arnold, and B. C. Clark, "Multi-frequency electrical impedance myography as a predictor of muscle function in older adults," *Muscle Nerve*, vol. 72, no. 3, pp. 416–423, Sep. 2025, doi: 10.1002/mus.28449.
